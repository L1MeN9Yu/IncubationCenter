...
public class AppleToastView: UIView {
    ...
    public init(child: UIView, minHeight: CGFloat = 58, minWidth: CGFloat = 150) {
...
extension AppleToastView: Toastable {
    public func createView(for toast: Toast) {
        ...
        DispatchQueue.main.async { self.style() }
    }
}

public extension AppleToastView {
    override func traitCollectionDidChange(_ previousTraitCollection: UITraitCollection?) {
        super.traitCollectionDidChange(previousTraitCollection)
        UIView.animate(withDuration: 0.5) { self.style() }
    }
}

private extension AppleToastView {
    func style() {
        layoutIfNeeded()
        clipsToBounds = true
