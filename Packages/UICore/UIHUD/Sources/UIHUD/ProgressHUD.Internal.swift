...
extension ProgressHUD {
    func setup(status: String? = nil, progress: CGFloat? = nil, animatedIcon: AnimatedIcon? = nil, staticImage: UIImage? = nil, hide: Bool, interaction: Bool) {
        setupNotifications()
...
private extension ProgressHUD {
    func setupNotifications() {
        if viewBackground == nil {
            NotificationCenter.default.addObserver(self, selector: #selector(setupPosition(_:)), name: Notification.Name.keyboardWillShow, object: nil)
            NotificationCenter.default.addObserver(self, selector: #selector(setupPosition(_:)), name: Notification.Name.keyboardWillHide, object: nil)
            NotificationCenter.default.addObserver(self, selector: #selector(setupPosition(_:)), name: Notification.Name.keyboardDidShow, object: nil)
            NotificationCenter.default.addObserver(self, selector: #selector(setupPosition(_:)), name: Notification.Name.keyboardDidHide, object: nil)
            NotificationCenter.default.addObserver(self, selector: #selector(setupPosition(_:)), name: Notification.Name.orientationDidChange, object: nil)
        }
    }
    ...
    func setupAnimatedIcon(_ animatedIcon: AnimatedIcon?) {
        viewProgress?.removeFromSuperview()
        viewAnimation?.removeFromSuperview()
        staticImageView?.removeFromSuperview()
    ...
    func setupPosition(_ notification: Notification? = nil) {
        ...
        if let notification = notification {
            ...
            if (notification.name == Notification.Name.keyboardWillShow) || (notification.name == Notification.Name.keyboardDidShow) {
                heightKeyboard = frameKeyboard.size.height
            } else if (notification.name == Notification.Name.keyboardWillHide) || (notification.name == Notification.Name.keyboardDidHide) {
                heightKeyboard = 0
            } else {
